...
public class Validate {
	...
	public boolean validateCurrency(String input) {
		if (input.matches("^[0-9.,]{1,20}$")) {
			return true;
		} else {
			JOptionPane.showMessageDialog(null, "Wrong currency format! Only digits . , allowed! 1–20 characters!");
			return false;
		}
	}

	public static boolean nameWithNumbers(String namn) {
		boolean isCorrect = false;
		if (namn.length() > 1 && namn.length() <= 25) {
			isCorrect = true;
		} else {
	...
	public static boolean authority(String text) {
		if (text.equals("1") || text.equals("2")) {
			return true;
		} else {
			JOptionPane.showMessageDialog(null, "Authority level must be 1 or 2, ex 1");
			return false;
		}

	}

	public static boolean responsibility(String text){
		if(text.isEmpty()){

			JOptionPane.showMessageDialog(null, "Area of responsibility can't be empty");
			return false;
		}
		return true;
	}
